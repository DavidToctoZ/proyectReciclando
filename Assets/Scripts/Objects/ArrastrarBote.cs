...
public class ArrastrarBote : MonoBehaviour
{
    ...
    void OnTriggerStay2D(Collider2D col)
    {
        ...
        if (bolResetPos == true && col.gameObject.tag == "usos")
        {
            if (gameObject.name == col.gameObject.GetComponent<SetMaterial>().material)
            {
                Destroy(gameObject);
